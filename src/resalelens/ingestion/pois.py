...
def ingest_pois(session: Session) -> dict[str, int]:
    ...
    summary = {"fetched": 0, "inserted": 0, "errors": 0, "warnings": 0}
        ...
        for poi_type, terms in search_terms.items():
            for term in terms:
                ...
                if batch:
                    ...
                    existing_query = select(POI.latitude, POI.longitude, POI.name).where(
                        POI.poi_type == poi_type
                    )
                    existing = session.execute(existing_query).all()
                    existing_set = {
                        (float(r.latitude), float(r.longitude), r.name) for r in existing
                    }
